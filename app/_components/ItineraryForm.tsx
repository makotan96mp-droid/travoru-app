...
const SUGGESTS_BY_CITY: Record<string, string[]> = {
  "大阪": ["ユニバーサル・スタジオ・ジャパン（USJ）", "カニ道楽 本店", "アメリカ村（アメ村）", "道頓堀", "大阪城", "梅田スカイビル 空中庭園"],
  "東京": ["浅草・浅草寺", "東京スカイツリー", "渋谷スクランブル交差点", "明治神宮", "チームラボボーダレス", "築地・豊洲"],
...
function CalendarIcon(props:{className?:string}) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
      ...
      strokeLinecap="round" strokeLinejoin="round" className={props.className ?? "h-5 w-5"}>
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        aria-label="日付を選択"
        onClick={openPicker}
        data-date-button className="icon-trailing"
      >
        <CalendarIcon />
      </button>
    </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        <button type="button" className="btn-ghost" onClick={onUp} aria-label="上へ">↑</button>
        <button type="button" className="btn-ghost" onClick={onDown} aria-label="下へ">↓</button>
        <button type="button" className="btn-ghost" onClick={onRemove} aria-label="削除">×</button>
      </div>
    </div>
  );
}
...
export default function ItineraryForm() {
  const [city, setCity] = useState<string>(()=> typeof window!=="undefined" ? (localStorage.getItem("city")||"") : "");
  ...
  useEffect(()=>{ if(!wantsStay) setHotelName(""); },[wantsStay]);
  ...
  const togglePurpose = (p:string)=> setPurposes(cur => cur.includes(p)? cur.filter(x=>x!==p): [...cur,p]);
  const toggleMustSee = (s:string)=> setMustSee(cur => cur.includes(s)? cur.filter(x=>x!==s): [...cur,s]);
  const addCustomSpot = ()=> { const s=customSpot.trim(); if(!s) return; setMustSee(cur=>uniq([...cur,s])); setCustomSpot(""); };
  const moveUp=(i:number)=>{ if(i<=0) return; setMustSee(cur=>{const c=[...cur]; [c[i-1],c[i]]=[c[i],c[i-1]]; return c;}); };
  const moveDown=(i:number)=>{ setMustSee(cur=>{ if(i>=cur.length-1) return cur; const c=[...cur]; [c[i+1],c[i]]=[c[i],c[i+1]]; return c;}); };
  ...
  async function handlePreview(){
    ...
    }catch(e:any){ setErrorMsg(e.message || "プレビューに失敗しました"); setPreview(null); }
  ...
  return (
    <div className="space-y-6 text-[15px]">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {["観光","ショッピング","グルメ","宿泊"].map(p=>{
            ...
            return (
              <button
                key={p} type="button" aria-pressed={active} onClick={()=>togglePurpose(p)}
      ...
      {wantsStay && (
        <div>
          <PurposeSummary purposes={purposes} onRemove={(p)=> setPurposes((prev)=> prev.filter(x=> x!==p))} />
<label className="block text-sm font-medium text-[color:var(--fg)]">ホテル名</label>
          <input
            ...
            className="ui-field focus-visible:ui-focus"
          />
          <p className="mt-1 text-xs text-[color:var(--fg-muted)]">※「宿泊」をOFFにすると自動的にクリアされます</p>
        </div>
      )}
      ...
      <div>
        <div className="flex items-center justify-between">
          <label className="block text-sm font-medium text-[color:var(--fg)]">行きたい場所（mustSee）</label>
          <span className="text-xs text-[color:var(--fg)]/50">順番＝優先順／巡る順（ドラッグ＆ドロップ対応）</span>
        </div>
        ...
        {citySuggests.length>0 && (
          <div className="mt-2 flex flex-wrap gap-2">
            {citySuggests.map(s=>{
              ...
              return (
                <button key={s} type="button" onClick={()=>toggleMustSee(s)}
                  className={`chip text-xs ${active ? "chip--active" : ""}`} title={active ? "リストから除外" : "リストに追加"}>
                  {s}
                </button>
              );
            })}
        ...
        <div className="mt-3 flex gap-2">
          <input
            value={customSpot} onChange={(e)=>setCustomSpot(e.target.value)}
            placeholder="スポット名を追加（例：通天閣）"
            className="ui-field focus-visible:ui-focus flex-1"
            onKeyDown={(e)=>{ if(e.key==="Enter"){ e.preventDefault(); addCustomSpot(); } }}
          />
          <button type="button" onClick={addCustomSpot}
          ...
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={onDragEnd}>
            <SortableContext items={mustSee} strategy={verticalListSortingStrategy}>
              <div className="mt-3 rounded-md border border-[color:var(--border-subtle)] divide-y divide-white/10">
                {mustSee.map((s,i)=>(
                  <SortableRow
                    key={s} id={s} index={i} title={s}
                    onUp={()=>moveUp(i)} onDown={()=>moveDown(i)} onRemove={()=>removeAt(i)}
                  />
                ))}
              </div>
            </SortableContext>
...
<button type="button" className="btn-ghost" onClick={() => { setCity(""); setStartDate(""); setEndDate(""); setPurposes([]); setHotelName(""); setMustSee([]); setPreview(null); }}>クリア</button>
        <button type="button" disabled={!canSubmit || loading} onClick={handlePreview} className="btn-primary">
          {loading ? "生成中..." : "プレビュー生成"}
        </button>
        {!canSubmit && <span className="text-sm text-[color:var(--fg-muted)]">都市・日付（＋宿泊ON時はホテル名）を入力してください</span>}
      </div>
          ...
          {preview.notes && (
            <div className="rounded-md border border-[color:var(--border-subtle)] p-3">
              <div className="text-sm font-medium mb-1">Notes</div>
                <div className="text-xs text-[color:var(--fg-muted)] mb-1">期間：{startDate} – {endDate}</div>
