...
const nearestFreeSlot = (items: DayPlanItem[], target: string) => {
  ...
  // どうしても空きが無ければ末尾の最後に
  let last = items.length ? items[items.length-1].time : "10:00";
...
export function clampMainPerDay(days: Days, caps: { [k: string]: number } = {}) {
  ...
  const keys = Object.keys(days).filter(k => /^day\d+$/.test(k)).sort((a,b)=>Number(a.slice(3))-Number(b.slice(3)));
