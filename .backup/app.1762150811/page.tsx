...
export default function Page() {
        ...
        <div className="mt-7 flex items-center gap-4">
          <a href="/new"
             className="inline-flex h-10 items-center rounded-full px-5 text-[15px] font-medium
                        ...
                        shadow-[0_6px_20px_rgba(0,0,0,.35)] backdrop-blur lift-1">
            今すぐプランを作る
          </a>
          <a href="#how-it-works"
             className="inline-flex h-10 items-center rounded-full px-5 text-[15px] font-medium
                        text-white/95 border border-white/30 bg-white/10 hover:bg-white/15
                        shadow-[0_2px_10px_rgba(0,0,0,.25)] backdrop-blur lift-1">
            使い方を見る
          </a>
        </div>
      </div>
  ...
  <StickySteps />
      ...
      {/* ===== 人気の目的地 ===== */}
      <section className="my-20 relative z-10 overflow-visible">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          {DESTS.map((d, i) => (
            <a
              key={d.href}
              href={d.href}
              className="group relative rounded-lg sm:rounded-xl overflow-hidden border border-white/35 bg-slate-900/60 shadow-[0_20px_40px_rgba(0,0,0,0.28)] ring-1 ring-white/25 ring-1 ring-white/10 transition hover:shadow-md cursor-pointer"
            >
